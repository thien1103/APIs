...
GetAllLeaveRequestsOnClassForTeacher(req, res) {
  try {
      ...
      const sql = `
          ...
          FROM \`leave\` l 
          LEFT JOIN date_ranges_request dr ON l.id = dr.requestId 
      ...
      pool.query(sql, sanitizedClasses, (err, results) => {
          ...
          // Organize leave requests by request ID
          ...
          results.forEach(row => {
              ...
              if (!leaveRequestsMap.has(id)) {
                  leaveRequestsMap.set(id, {
                      id,
                      user_id,
                      title,
                      content,
                      status: status === 1 ? "Đã duyệt" : "Chưa duyệt",
                      dateRangeList: [],
              ...
              // Get the request object and add date range (max 3)
              const request = leaveRequestsMap.get(id);
              if (request.dateRangeList.length < 3) {
                  request.dateRangeList.push({
                      date,
                      session: {
          ...
          res.status(200).json({
              ...
              data: Array.from(leaveRequestsMap.values()), // Convert map to array
          });
      });
  } catch (err) {
      console.error(err);
